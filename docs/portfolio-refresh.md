...
- `src/pages/Home.jsx`: supplied biography, three research interests, collaboration invitation, and links to evidence-backed skills.
...
`/statement` is canonical. `/research` and `/research/` redirect to it while retaining the query and fragment. Homepage interests link to the statement's research-background sections. The statement overview and table of contents link to unique section IDs, including repeated subtitles.
